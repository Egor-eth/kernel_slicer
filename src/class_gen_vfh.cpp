...
public:
  ...
  bool VisitMemberExpr_Impl(clang::MemberExpr* expr) override
  {
    ...
    if((thisTypeName == m_className || thisTypeName == m_interfaceName) && WasNotRewrittenYet(expr))
    {
      ...
      kslicer::ReplaceTextMacroSafe(m_rewriter, expr->getSourceRange(), exprReplaced);
      MarkRewritten(expr);
    }
    else if(dataClassNames.find(thisTypeName) != dataClassNames.end() && WasNotRewrittenYet(expr))
    {
      ...
      kslicer::ReplaceTextMacroSafe(m_rewriter, expr->getSourceRange(), prefix + fieldName);
      MarkRewritten(expr);
    }  
    else if(expr->isArrow() && WasNotRewrittenYet(expr))
    {
      ...
      kslicer::ReplaceTextMacroSafe(m_rewriter, expr->getSourceRange(), kslicer::GetRangeSourceCode(base->getSourceRange(), m_compiler) + "." + memberName);
      MarkRewritten(expr);
    }
